...
import { buildPlanningCapabilityRows, buildPlanningSnapshot, buildPlanningTrendData, filterPlanningWeeks } from '../services/analytics';
...
export default function KpiDashboardPage() {
  ...
  const planning = buildPlanningSnapshot(uploadedFiles, roles, selectedWeekScope);
  const capabilityRows = buildPlanningCapabilityRows(uploadedFiles, roles, selectedWeekScope);
  const trend = filterPlanningWeeks(buildPlanningTrendData(uploadedFiles, roles, selectedWeekScope), selectedWeekIndices);
  ...
  const roleCards = useMemo(() => capabilityRows.map(row => {
    ...
    const achievedWeeks = selectedWeekIndices.filter(weekIndex => {
      const weekRows = buildPlanningCapabilityRows(uploadedFiles, roles, [weekIndex]);
  ...
  const downloadPowerPoint = async () => {
    await exportKpiPowerPoint({
      label: selectedLabel,
      requiredHours: planning.requiredHours,
      scheduledHours: planning.scheduledHours,
      capability: planning.capability,
      variance: planning.variance
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} className="card-grid">
        <Grid item xs={12} sm={6} md={3}><Paper className="kpi-card" sx={{ height: '100%' }}><Typography variant="overline">Required Hours</Typography><Typography variant="h4" sx={{ fontWeight: 800 }}>{Math.ceil(planning.requiredHours)}</Typography><Typography variant="caption">Selected scope</Typography></Paper></Grid>
        <Grid item xs={12} sm={6} md={3}><Paper className="kpi-card" sx={{ height: '100%' }}><Typography variant="overline">Scheduled Hours</Typography><Typography variant="h4" sx={{ fontWeight: 800 }}>{Math.ceil(planning.scheduledHours)}</Typography><Typography variant="caption">Selected scope</Typography></Paper></Grid>
        <Grid item xs={12} sm={6} md={3}><Paper className="kpi-card" sx={{ height: '100%' }}><Typography variant="overline">Overall KPI</Typography><Typography variant="h4" sx={{ fontWeight: 800 }}>{planning.capability.toFixed(1)}%</Typography><Typography variant="caption">Scheduled / required</Typography></Paper></Grid>
        <Grid item xs={12} sm={6} md={3}><Paper className="kpi-card" sx={{ height: '100%' }}><Typography variant="overline">Hour Variance</Typography><Typography variant="h4" sx={{ fontWeight: 800 }}>{Math.ceil(planning.variance)}</Typography><Typography variant="caption">Scheduled minus target</Typography></Paper></Grid>
      </Grid>
